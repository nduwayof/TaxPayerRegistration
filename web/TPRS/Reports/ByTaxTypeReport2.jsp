...
<%@page import="com.djuma.NonIndividual_Representative"%>
<%@page import="com.djuma.Nonindividual_TaxType"%>
...
<html>
    ...
    <body>
        <%
            String value=request.getParameter("taxType");
            String type="";
            String code="";
          ...
          //  String itn = request.getParameter("itnm");
           ...
           //image.scaleAbsolute(400, 50);
            ...
            Paragraph par = new Paragraph(new Chunk( " "+   "Report of ["+code+"] "+type+" TaxPayers on " + da , font2));
            ...
            tablee1.addCell(new Phrase(new Chunk(" Enterprise Type ", FontFactory.getFont(FontFactory.TIMES_BOLD, 13, Font.BOLD, BaseColor.MAGENTA))));
            PdfPTable tablee2 = new PdfPTable(1);
            tablee2.addCell(new Phrase(new Chunk(" Registered Name", FontFactory.getFont(FontFactory.TIMES_BOLD, 13, Font.BOLD, BaseColor.MAGENTA))));
            ...
            tablee4.addCell(new Phrase(new Chunk(" Country Of Residence", FontFactory.getFont(FontFactory.TIMES_BOLD, 13, Font.BOLD, BaseColor.MAGENTA))));
            ...
            try {
              //  String itnm = request.getParameter("username");
                ...
                for(TaxType t: TaxType.listTaxType()){
                   if(t.getId()==Integer.parseInt(value)){ 
                    for(Nonindividual_TaxType s: Nonindividual_TaxType.list()){
                    if(s.getTaxTypeId()==t.getId()){
                for(NonIndividual in: NonIndividual.listNonIndividual()) {
                   if(in.getNonIndividualId()==s.getNonIndividualId()&&in.isHasInfo()&&in.isDeregistered()==false){
                    ...
                    table1.addCell(new Phrase(new Chunk(in.getEnterpriseType(), FontFactory.getFont(FontFactory.TIMES_BOLD, 9, Font.BOLD, BaseColor.BLACK))));
                    

                    PdfPTable table2 = new PdfPTable(1);
                    table2.addCell(new Phrase(new Chunk(in.getRegisteredName(), font5)));

                    PdfPTable table3 = new PdfPTable(1);
                    table3.addCell(new Phrase(new Chunk(in.getTinNumber(), font5)));

                    PdfPTable table4 = new PdfPTable(1);
                    table4.addCell(new Phrase(new Chunk(in.getCountryOfResidence(), font5)));
                      PdfPTable table5 = new PdfPTable(1);
                   for(NonIndividual_Representative ir:NonIndividual_Representative.list()){
                         if(ir.getNonIndividualId()==in.getNonIndividualId()){
